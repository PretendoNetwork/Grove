(function (a, b) {
	...
	f.extend({
		...
		ajax: function (a, c) {
			...
			if (E.promise(D).complete = v.add, D.success = D.done, D.error = D.fail, s.url = ((a || s.url || qa) + "").replace(xd, "").replace(Ad, za[1] + "//"), s.type = c.method || c.type || s.method || s.type, s.dataTypes = f.trim(s.dataType || "*").toLowerCase().match(ga) || [""], null == s.crossDomain && (d = sc.exec(s.url.toLowerCase()), s.crossDomain = !(!d || d[1] ===
					za[1] && d[2] === za[2] && (d[3] || ("http:" === d[1] ? 80 : 443)) == (za[3] || ("http:" === za[1] ? 80 : 443)))), s.data && s.processData && "string" != typeof s.data && (s.data = f.param(s.data, s.traditional)), ka(uc, s, c, D), 2 === N) return D;
						...
						try {
							if (g && (d || 4 === n.readyState))
								if (g = b, j && (n.onreadystatechange = f.noop, Mb && delete Ja[j]), d) 4 !== n.readyState && n.abort();
									...
									m || !c.isLocal || c.crossDomain ? 1223 === m && (m = 204) : m = s.text ? 200 : 404
								}
						} catch (l) {
							d || e(-1, l)
						}
